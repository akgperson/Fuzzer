...
def random_real():
#add possibility of it being a 0
    y = 0
...
while assertions > 0:
    ...
    if random.random() < 0.33: #new int
        if random.random() < 0.3:
            ...
            d['Int'].append(new_int)
    if random.random() < 0.33: #new int by exp
        p = random.random()
        if p < 0.4:
    ...
    if random.random() < 0.33: #new boolean
    ...
    if random.random() < 0.33: #new real
        if random.random() < 0.5:
            ...
            new_real = random_real()
            d['Real'].append(new_real)
    if random.random() < 0.33: #new real from expression
    ...
    if random.random() < 0.33: #new bool from reals
        #n-array or binary?
        operands = str(random.choice(d['Real']))
    ...
    #decide on possibilities to assert
    #choose variable
    if random.random() < 0.5:
